...
def addItem(productName: hug.types.text, productDescription: hug.types.text, productImg:hug.types.text, contactEmail: hug.types.text):
        ...
        productQr = productName + '/' + contactEmail
...
def getItem(productQr: hug.types.text):
        ...
        cursor.execute("select * from dbo.product where product_qr =?", productQr)
        list = cursor.fetchone()
        productName = list[0]
        productDescription = list[1]
        productImg = list[4]
...
#         getItem('Janet\'s Jamsjennyjam@gmail.com')
